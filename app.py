...
def splash():
    ...
    message += 'This Azure service can be stopped/started/rebooted at any time. When that hapepens everything is lost. You will need to re-upload you DMN compliant Excel workbooks in order to restore services. '
...
def show_decision_service_part(decisionServiceName, part):
    ...
    if part == 'glossary':          # Show the Glossary for this Decision Service
        ...
        message += '<div align="center">[curl {}:5000{}]</div>'.format(urlparse(request.base_url).hostname, url_for('download_decision_service_api', decisionServiceName=decisionServiceName))
        message += '<p align="center"><b><a href="{}">{}</a></b></p></body></html>'.format(url_for('show_decision_service', decisionServiceName=decisionServiceName), ('Return to Decision Service ' + decisionServiceName).replace(' ','&nbsp;'))
        return message
